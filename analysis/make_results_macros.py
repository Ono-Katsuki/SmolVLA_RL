...
def _evala_recompute(eval_a: dict) -> dict:
    ...
    Returns per-arm {rate, ci (marginal), and for non-sft arms: net, diff_ci
    (task-clustered), diff_ci_bonf (Bonferroni 0.05/3), mcnemar_p}. Every stored
    ...
    for m in succ:
        ...
        if b is not None and c is not None:
            ...
            p = None
        out[m].update({"net": net, "diff_ci": diff_ci,
...
def build_evalA_macros(eval_a: dict) -> list[str]:
    ...
    # paired-vs-SFT: McNemar p, net paired wins, task-clustered per-task-diff CI (pp)
    for prefix, key in _EVALA_PAIRED:
        r = rc.get(key)
        if r and "diff_ci" in r:
            p, net, ci = r["mcnemar_p"], r["net"], r["diff_ci"]
...
def build_evalA(eval_a: dict) -> str:
    """booktabs primary-eval table: Method | marginal success (CI) |
    paired Delta vs SFT with the task-clustered 95% CI (the primary statistic) |
    ...
    L = ["% AUTO-GENERATED by analysis/make_results_macros.py --- do not hand-edit.",
         ...
         "Method & success (task-boot.\\ 95\\% CI) "
         "& paired $\\Delta$ vs.\\ SFT: net (task-clustered 95\\% CI) "
         "& McNemar $p$ \\\\", "\\midrule"]
    for method in mrt.METHOD_ORDER:
        ...
        if method == "sft":
            L.append(f"{label} & {succ_cell} & --- (reference) & --- \\\\")
            continue
        rcm = rc.get(method, {})
        ...
        ci_s = f"[${_spp(dci[0])},{_spp(dci[1])}$]\\,pp" if dci_ok else ""
        p_s = f"${p:.3f}$" if p is not None else "---"
        L.append(f"{label} & {succ_cell} & {net_s} {ci_s}{sig} & {p_s} \\\\")
